...
const MobileSheet: React.FC<MobileSheetProps> = ({
  ...
  const variants = React.useMemo(() => {
    ...
    switch (side) {
      case 'bottom':
        return { ...base, initial: { ...base.initial, y: '100%' }, animate: { ...base.animate, y: 0 }, exit: { ...base.exit, y: '100%' } }
      case 'top':
        return { ...base, initial: { ...base.initial, y: '-100%' }, animate: { ...base.animate, y: 0 }, exit: { ...base.exit, y: '-100%' } }
      case 'left':
        return { ...base, initial: { ...base.initial, x: '-100%' }, animate: { ...base.animate, x: 0 }, exit: { ...base.exit, x: '-100%' } }
      case 'right':
        return { ...base, initial: { ...base.initial, x: '100%' }, animate: { ...base.animate, x: 0 }, exit: { ...base.exit, x: '100%' } }
    }
  }, [side])
  ...
  const handleDragEnd = (_event: unknown, info: PanInfo) => {
    setIsDragging(false)
    if (onDragEndProp) { onDragEndProp(info); return }
    if (!dismissible || preventClose) return

    const threshold = 50
    let shouldClose = false
    switch (side) {
      case 'bottom': shouldClose = info.offset.y > threshold || info.velocity.y > 500; break
      case 'top': shouldClose = info.offset.y < -threshold || info.velocity.y < -500; break
      case 'left': shouldClose = info.offset.x < -threshold || info.velocity.x < -500; break
      case 'right': shouldClose = info.offset.x > threshold || info.velocity.x > 500; break
    }
  ...
  const dragConstraints = React.useMemo(() => {
    switch (side) {
      case 'bottom': return { top: 0, bottom: 200 }
      case 'top': return { top: -200, bottom: 0 }
      case 'left': return { left: -200, right: 0 }
      case 'right': return { left: 0, right: 200 }
    }
  }, [side])
        ...
        >
          ...
          >
            {showHandle && (side === 'bottom' || side === 'top') && (
              <div className={cn('flex justify-center py-3 cursor-grab active:cursor-grabbing', side === 'top' ? 'order-last' : '')}>
